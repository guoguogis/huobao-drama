# 🎬 Juwei Video Creation Platform - AI ショートドラマ生成プラットフォーム
...
<h2>🔑 <a href="https://api.firemux.com">Juwei API Key を取得 👉 今すぐ見る</a></h2>

**テキスト・画像・動画のすべての AI 機能を、1 つの Key で有効化**

デプロイ後、「設定 → 剧威クイック設定」に Key を貼るだけで、3 つの推奨設定をワンクリックで書き込み
...
Juwei Video Creation Platform は AI ベースのショートドラマ自動制作プラットフォームです。脚本生成、キャラクターデザイン、分镜制作から動画合成まで、全工程を自動化します。
...
| `PUBLIC_BASE_URL` | — | 本バックエンドの公開 URL。設定するとローカルの参照動画/音声は**署名付き URL** として上流 API に渡されます（サーバーデプロイ用）。未設定なら Base64 でインライン化 |
| `MEDIA_REF_MODE` | `auto` | ローカル参照動画/音声を上流へ渡す方式：`auto`（`PUBLIC_BASE_URL` 未設定なら Base64 インライン、設定済みなら署名 URL）、`inline`、`public` |
| `MEDIA_INLINE_MAX_MB` | `15` | Base64 インライン化できる 1 ファイルの上限サイズ。超えると署名 URL にフォールバック |
| `MEDIA_URL_TTL_HOURS` | `6` | 上流に渡す署名付き素材 URL の有効期間 |
...
1. 「設定」ページを開く
2. 「剧威クイック設定」に Juwei API Key を貼る（[api.firemux.com で取得](https://api.firemux.com)）。テキスト・画像・動画の 3 つの推奨設定をワンクリックで書き込み
...
設定ページの「剧威クイック設定」に API キーを貼り付けると推奨 3 設定を一括書き込み。手動テンプレートでの追加も可能です。使用モデルは上部バーでいつでも切り替えられます（ステップ 5 参照）。
...
> ローカル参照素材について：`PUBLIC_BASE_URL` 未設定の場合、参照**オーディオ**は Base64 の `data:` URL としてインライン化されます（モデル提供元はあなたのマシンに到達できないため、ローカル開発で唯一動作する方法です）。参照**動画**はサイズが大きいため明確なエラーになります。サーバーにデプロイして `PUBLIC_BASE_URL` を設定すると、素材は**署名付き URL** として上流に渡され、公開状態にする必要はありません。テキスト→動画・画像生成など他の機能には影響しません。
...
A: 初回デプロイ時の正常な案内です。「設定」ページで「剧威クイック設定」に API Key を貼ってワンクリックで書き込むか、「手動テンプレート」でプロバイダーを追加してください。テキスト・画像・動画の 3 類すべてに有効な設定があるとバナーは自動で消えます。
...
- 初回利用ガイド
  ...
  - 設定ページに「剧威クイック設定」を追加：1 つの Key でテキスト/画像/動画の 3 つの推奨設定を書き込み
